...
class App extends Component {
	...
	getCurrentParticipants() {
		this.getParticipants(apis.tournaments, this.tournamentName)
			.then(res => {
				let participants = [];
				res.forEach(p => {
	...
	getParticipants(api, tournamentName) {
		...
		const url = `https://cors-anywhere.herokuapp.com/https://api.challonge.com/v1/${api}/${tournamentName}/participants.json?api_key=${
			this.API_KEY
		}`;
		...
		const liveMatches = this.state.matches.map(c => {
			...
			if (c.match.state === 'open' && c.match.underway_at !== null) {
				...
				return (
					<div>
						<div key={1}>
							<div key={2} style={heatTopBannerContainer}>
							...
							>
								<div
									className="teamOneHeatPanel"
									style={matchHeatContainer}
								>
									...
									<div className="teamOneScoreContainer">
										...
										<div style={teamOneHeatBlock}>
											{heatTotals[0] || ''}
										</div>
									</div>
								</div>
								<div
									className="teamTwoHeatPanel"
									style={matchHeatContainerTwo}
								>
